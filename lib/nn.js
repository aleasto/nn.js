...
class NeuralNetwork {
    constructor(){
        ...
        for(let i=0; i<this.hiddenLayers.length; i++){
            this.hiddenLayers[i].initWeights(arguments[arguments.length-1]);
            this.hiddenLayers[i].initBiases(arguments[arguments.length-1]);
        }
    ...
    mse(inputValues, targetValues){
        ...
        sum = sum/(2*this.outputLayer.neurons.length);
        return sum;
    }
    ...
    backprop(targetValues, learnRate){
        ...
        for(let k=0; k<this.outputLayer.neurons.length; k++){
            this.outputLayer.deltas[k] = -1 * (targetValues[k] - this.outputLayer.neurons[k]) * this.derivative(this.outputLayer.neurons[k]) / this.outputLayer.neurons.length;
        }
...
class Layer {
    constructor(n, prev){
        ...
        // Setup GPU
        this.evalNext = gpu.createKernel(function(n, w, b) {
            let sum = 0;
            for (let i = 0; i < this.constants.size; i++) {
                sum += n[i] * w[i][this.thread.x];
            }
            sum += b[this.thread.x];
            return activation(sum);
        })
        .setConstants({size:this.neurons.length});
    }
    ...
    feed(x, activation){
        ...
        this.evalNext = this.evalNext
            .setOutput([this.next.neurons.length])
            .setFunctions([activation]);

        let outputs = this.evalNext(this.neurons, this.next.weights, this.next.biases);
...
const logistic = {
    ...
    derivative: function(logistic_x){ return logistic_x * (1 - logistic_x) }
}

const tanh = {
    activation: function(x){ return Math.tanh(x) },
    derivative: function(tanh_x){ return 1 - Math.pow(tanh_x, 2) }
}

const relu = {
    activation: function(x){ return Math.max(0,x) },
    derivative: function(relu_x){ return relu_x == 0 ? 0 : 1 }
}
